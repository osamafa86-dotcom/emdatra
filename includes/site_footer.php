<?php
/** Shared site shell — footer, modals and scripts. */
$isHome  = ($pageActive ?? '') === 'home';
$topHref = $isHome ? '#home' : '#header';
$titleAr = $titleAr ?? ($seo['title_ar'] ?? 'إمداترا | حلول الاستيراد والتصدير');
$titleEn = $titleEn ?? ($seo['title_en'] ?? 'emdatra | Import & Export Solutions');
$F = __DIR__ . '/..';
?>
  <footer class="footer">
    <div class="container">
      <div class="footer__top">
        <div class="footer__brand">
          <a href="<?= $isHome ? '#home' : 'index.php' ?>" class="logo logo--footer" aria-label="<?= esc($brand['name'] ?? 'emdatra') ?>">
        ...
        <nav class="footer__col" aria-label="روابط سريعة">
          ...
          <?php foreach ($nav as $item): ?>
            <a href="<?= esc(nav_href($item['href'] ?? '#', $isHome)) ?>" data-ar="<?= esc($item['ar'] ?? '') ?>" data-en="<?= esc($item['en'] ?? '') ?>"><?= esc($item['ar'] ?? '') ?></a>
          <?php endforeach; ?>
        </nav>
        ...
        <nav class="footer__col" aria-label="خدماتنا">
          ...
          <?php foreach (array_slice($servicesItems, 0, 4) as $s): ?>
            <a href="<?= esc(nav_href('#services', $isHome)) ?>" data-ar="<?= esc($s['title_ar'] ?? '') ?>" data-en="<?= esc($s['title_en'] ?? '') ?>"><?= esc($s['title_ar'] ?? '') ?></a>
          <?php endforeach; ?>
        </nav>
      ...
      <div class="footer__bar">
        ...
        <div class="footer__legal">
          <a href="catalog.php" data-ar="كتالوج السلع" data-en="Catalog">كتالوج السلع</a>
  ...
  <a href="<?= esc($topHref) ?>" class="to-top" id="toTop" aria-label="العودة للأعلى">
  ...
  <script>window.EMDATRA_TITLES={ar:<?= json_encode($titleAr, JSON_UNESCAPED_UNICODE) ?>,en:<?= json_encode($titleEn, JSON_UNESCAPED_UNICODE) ?>};</script>
  <script src="js/main.js?v=<?= @filemtime($F . '/js/main.js') ?: '1' ?>"></script>
  <script src="js/chat.js?v=<?= @filemtime($F . '/js/chat.js') ?: '1' ?>"></script>
